        ...
        for node in nodes:
            ...
            if cursors.exists():
                for cursor in cursors:
                    ...
                    last_eid = getattr(cursor, "last_event_id", None)
                    last = str(last_eid) if last_eid is not None else "none"
                    self.stdout.write(
                        f"  Cursor [{tenant}]: bootstrapped={bootstrapped} last_event={last}"
                    )
            else:
                self.stdout.write("  Cursor: none")

            # Pending deliveries
            pending = EventDelivery.objects.filter(node=node, status=DeliveryStatus.PENDING).count()
            failed = EventDelivery.objects.filter(node=node, status=DeliveryStatus.FAILED).count()
